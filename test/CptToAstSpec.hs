...
cptToAstDefineSpec = do
  describe "CptToAst Define" $ do
    ...
    it "case define x #f" $ do
      cptToAst (CptLists[CptLists[CptSymbols "define", CptSymbols "x", CptSymbols "#f"]])
    ...
    it "case define (a) #t" $ do
      cptToAst (CptLists[CptLists[CptSymbols "define", CptLists[CptSymbols "a"], CptSymbols "#t"]])
  ...
  describe "CptToAst Define with suger" $ do
    ...
    it "case x = 'hello'" $ do
      cptToAst (CptLists[CptLists[CptSymbols "x", CptSymbols "=", CptSymbols "hello"]])
        `shouldBe` [AstDefine (Left "x") (AstSymbol "hello")]
    it "case x = #f" $ do
      cptToAst (CptLists[CptLists[CptSymbols "x", CptSymbols "=", CptSymbols "#f"]])
        `shouldBe` [AstDefine (Left "x") (AstBoolean "#f")]
    it "case (a) = 12" $ do
    ...
    it "case (a) = 'world'" $ do
      cptToAst (CptLists[CptLists[CptLists[CptSymbols "a"], CptSymbols "=", CptSymbols "world"]])
        `shouldBe` [AstDefine (Right ["a"]) (AstSymbol "world")]
    it "case (a) = #t" $ do
      cptToAst (CptLists[CptLists[CptLists[CptSymbols "a"], CptSymbols "=", CptSymbols "#t"]])
        `shouldBe` [AstDefine (Right ["a"]) (AstBoolean "#t")]
...
cptToAstCallSpec = do
  describe "CptToAst Call" $ do
    ...
    it "case define (x) (#f)" $ do
      cptToAst (CptLists[CptLists[CptSymbols "define", CptLists[CptSymbols "x"], CptLists[CptSymbols "#f"]]])
    ...
    it "case (#t)" $ do
      cptToAst (CptLists[CptLists[CptSymbols "#t"]])
    ...
    it "case (or #t #t)" $ do
      cptToAst (CptLists[CptLists[CptSymbols "or", CptSymbols "#t", CptSymbols "#t"]])
  ...
  describe "CptToAst Call with sugar" $ do
    ...
    it "case (x) = ('hello')" $ do
      cptToAst (CptLists[CptLists[CptLists[CptSymbols "x"], CptSymbols "=", CptLists[CptSymbols "hello"]]])
        `shouldBe` [AstDefine (Right ["x"]) (AstCall [AstSymbol "hello"])]
    it "case (x) = (#f)" $ do
      cptToAst (CptLists[CptLists[CptLists[CptSymbols "x"], CptSymbols "=", CptLists[CptSymbols "#f"]]])
        `shouldBe` [AstDefine (Right ["x"]) (AstCall [AstBoolean "#f"])]
    ...
    it "case (#t && #t)" $ do
      cptToAst (CptLists[CptLists[CptSymbols "#t", CptSymbols "and", CptSymbols "#t"]])
...
cptToAstLambdaSpec = do
  describe "CptToAst Lambda" $ do
    ...
    it "case ((lambda (a b) (+ a b)) 'hello' #t)" $ do
      cptToAst (CptLists[CptLists[CptLists[CptSymbols "lambda", CptLists[CptSymbols "a", CptSymbols "b"], CptLists[CptSymbols "+", CptSymbols "a", CptSymbols "b"]], CptSymbols "hello", CptSymbols "#t"]])
